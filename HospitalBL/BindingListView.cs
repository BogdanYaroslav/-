...
namespace HospitalBL
{
    public class BindingListView<T> : BindingList<T>, IBindingListView
    {
        ...
        public BindingListView(IEnumerable<T> list)
        {
            if (!(list is null))
        ...
        protected override void OnListChanged(ListChangedEventArgs e)
        {
            ...
            {
                BindingListView<T> originalCopy = new BindingListView<T>(OriginalList);

                if (IsSortedCore)
                    originalCopy.ApplySortCore(SortPropertyCore, SortDirectionCore);

                if (!string.IsNullOrEmpty(Filter))
                    originalCopy.Filter = this.Filter;

                OriginalList.Remove(originalCopy[e.NewIndex]);
            }
        ...
        protected override int FindCore(PropertyDescriptor prop, object key)
        {
            ...
            if (key != null)
            {
                ...
                {
                    item = (T)Items[i];
                    if (propInfo.GetValue(item).Equals(key))
                        return i;
                }
            }
            return -1;
        ...
        BindingListView<T> unsortedItems;
        bool isSortedValue;
        ...
        protected override void ApplySortCore(PropertyDescriptor prop, ListSortDirection direction)
        {
            ...
            if (interfaceType != null)
            {
                ...
                unsortedItems = new BindingListView<T>();
